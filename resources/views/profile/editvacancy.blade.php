        ...
        <div class="container-fluid">
            <div class="row">
                ...
                <div class="col-lg-9 col-md-12 col-xs-12" style="border: 1px solid #eeeeee; padding: 20px;">
                    ...
                    <form method="post" action="{{ route('vacancy.update', ['vacancy' => $vacancy]) }}">
                        @csrf
                        @method('put')
                        <div class="row mb-3">
                            <div class="col-md-6">
                                <label for="name" class="col-form-label text-md-end">{{ __('Nomlanishi') }}</label>
                                <input id="name" type="text" class="name form-control" name="name" value="{{ $vacancy->name }}">
                            </div>
                        </div>
                        <div class="row mb-3">
                            <div class="col-md-3">
                                ...
                                <select name="specialist_id" id="specialist_id" class="form-control">
                                    ...
                                    @foreach($specialists as $specialist)
                                        <option value="{{ $specialist->id }}" @if($vacancy->specialist_id == $specialist->id) selected @endif data-id="{{ $specialist->id }}">{{ $specialist->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-md-3">
                                ...
                                <select name="skill" id="skill" class="form-control">
                                    ...
                                    @foreach($skills as $skill)
                                        <option value="{{ $skill->id }}" @if($vacancy->skill == $skill->id) selected @endif data-id="{{ $skill->id }}">{{ $skill->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-md-3" id="salary_hidden">
                                <label for="salary" class="col-form-label text-md-end">{{ __('Oylik maosh (so‘m)') }}</label>
                                <input id="salary" type="text" class="salary form-control" name="salary" value="{{ $vacancy->salary }}">
                            </div>
                            <div class="col-md-3 mt-4">
                                <label class="col-form-label text-md-end">
                                    <input type="checkbox" id="is_salary" name="is_salary" value="{{ $vacancy->is_salary }}" class="hide_salary"><span> {{ __('Oylik kelishlgan holda') }}</span>
                                </label>
                            </div>
                        </div>
                        ...
                        <div class="row mb-3">
                            <div class="col-md-12">
                                ...
                                <textarea name="desc" id="desc" cols="30" rows="10">{{ $vacancy->desc }}</textarea>
                            </div>
                        </div>
