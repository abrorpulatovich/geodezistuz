                    ...
                    <div class="card-body">
                        @if(Auth::user()->status == 1)
                            <div class="btn-group d-grid gap-2 d-md-flex justify-content-md-end">
                                @if($citizen_status == 2)
                                    <a href="{{ route('rezumes.create') }}">
                                        <button class="btn btn-success" type="button"><i class="bi bi-plus-lg"></i>
                                            Rezume qo‘shish
                                        </button>
                                    </a>
                                @endif
                                <a href="{{ route('citizens.show', ['citizen' => $citizen_id]) }}">
                            ...
                            @if(count($rezumes) > 0)
                                ...
                                @foreach($rezumes as $rezume)
                                    <tr>
                                        ...
                                        <td>
                                            <div class="btn-group">
                                                <a href="{{ route('rezumes.show', ['rezume' => $rezume->id]) }}"
                                                   class="btn btn-info"><i class="bi bi-eye"></i></a>
                                                @if(Auth::user()->status == 1)

                                                    <form
                                                        action="{{ route('rezumes.destroy', ['rezume' => $rezume->id]) }}"
                                                        method="POST">
                                                        @csrf
                                                        @method('DELETE')
