...
@section('content')
    ...
    <div class="content-wrapper">
        ...
        <section class="content-header">
            ...
            <ol class="breadcrumb">
                <li><a href="{{ route('home_page') }}"><i class="fa fa-dashboard"></i> {{ __('messages.Main') }}</a>
                </li>
            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="box box-primary">
                        ...
                        <div class="box-body">
                            <p><a href="{{ route('category.create') }}" class="btn btn-primary"><i
                                        ...
                                        <td>
                                            <select name="parent_id" id="filter_parent_id" class="form-control">
                                                ...
                                                @foreach($parent_categories as $key => $pc)
                                                    <option value="{{ $key }}"
                                                            @if(request()->get('parent_id') == $key) selected @endif>{{ $pc }}</option>
                                                @endforeach
                                            </select>
                                        </td>
                                        <td><input type="text" name="name" id="filter_name" class="form-control"
                                                   value="{{ request()->get('name') }}"></td>
                                        <td><input type="text" name="slug" id="filter_slug" class="form-control"
                                                   value="{{ request()->get('slug') }}"></td>
                                        <td>
                                            <select name="status" id="filter_status" class="form-control">
                                                <option value="">---</option>
                                                <option value="2"
                                                        @if(request()->get('status') == 2) selected @endif>{{ __('messages.Active') }}</option>
                                                <option value="1"
                                                        @if(request()->get('status') == 1 and request()->get('status') != null) selected @endif>{{ __('messages.Inactive') }}</option>
                                            </select>
                                        </td>
                                        <td></td>
                                    </tr>
